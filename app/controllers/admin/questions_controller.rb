class Admin::QuestionsController < Admin::ApplicationController
  ...
  def index
    @admin_questions = Question.includes(:user, :category).all
  end
  ...
  private
    ...
    def set_admin_question
      @admin_question = Question.friendly.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def admin_question_params
      params.require(:question).permit(:user_id, :category_id, :body, :slug)
    end
end
